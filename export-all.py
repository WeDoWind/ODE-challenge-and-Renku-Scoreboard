...
def get_remote_directories() -> List[str]:
    ...
    try:
        rc, out, err = run_command(
            [str(Config.RCLONE), "lsjson", Config.REMOTE, "-R", "--dirs-only"]
        )
        if rc != 0:
...
def sync_submissions(participant_path: str) -> bool:
    ...
    local_path = (
        Config.LOCAL_ROOT / participant_path.replace("/", os.sep) / "Submissions"
    )
...
def push_to_github():
    ...
    try:
        run_command(
            [str(Config.GIT), "add", str(Config.DEPLOY_DIR / "rmse_ranking.csv")],
            check=True,
        )
